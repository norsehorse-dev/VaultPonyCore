//! exFAT up-case table (spec 7.2), identity-run compressed.
//!
//! Kept as a Rust source array rather than a committed .bin blob:
//! F-Droid's source scanner rejects binary files in the tree, and
//! scanignore is not accepted. The bytes are the canonical compressed
//! table, unchanged. Used only by the exFAT formatter (format.rs).

pub(crate) static UPCASE_TABLE: [u8; 5836] = [
