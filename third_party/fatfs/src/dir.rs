...
impl<'a, T: ReadWriteSeek + 'a> Dir<'a, T> {
    ...
    fn write_entry(&self, name: &str, raw_entry: DirFileEntryData) -> io::Result<DirEntry<'a, T>> {
        ...
        // VaultPony patch (mirrors upstream master, unreleased): the special
        // "." and ".." entries must be bare 8.3 entries — never LFN.
        // fsck.fat flags and Windows rejects directories whose dot entries
        // carry LFN records. See third_party/fatfs/VAULTPONY.md.
        let lfn_utf16 = if name == "." || name == ".." {
            Self::encode_lfn_utf16("")
        } else {
